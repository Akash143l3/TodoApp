export default function LoadingSkeleton() {
    ...
    const skeletonItems = Array.from({ length: count }, (_, index) => (
        <div key={index} className="pt-20 px-3 py-4 border border-neutral-200 transition-all duration-200 rounded-md flex flex-col gap-3">
       ...
       return (
        <div className="w-screen grid grid-cols-3 gap-4">{skeletonItems}</div>
  );
}